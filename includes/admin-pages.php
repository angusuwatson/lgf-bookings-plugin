...
function simple_hotel_crm_import_motopress_bookings() {
    ...
    // Index raw bookings by ID for later lookups
    $all_raw_by_id = [];
    foreach ( $all_bookings as $b ) {
        $all_raw_by_id[ (string) ( $b['id'] ?? '' ) ] = $b;
    }
    ...
    foreach ( $all_bookings as $raw_booking ) {
        ...
        if ( $existing ) {
            ...
            if ( 0 === $has_rooms ) {
                $orphan_ids_for_retry[] = $external_id;
            }
            $analysis_skipped++;
            continue;
        }
    ...
    if ( ! empty( $new_booking_rows ) ) {
        ...
        $stats['skipped'] = $analysis_skipped;
    }

    // ── Phase 5: Retry room import for orphan bookings (non-destructive) ──
    $retry_imported = 0;
    $retry_errors = [];
    foreach ( $orphan_ids_for_retry as $external_id ) {
        $existing_booking = $wpdb->get_row( $wpdb->prepare(
            "SELECT id FROM {$bookings_table} WHERE source_booking_id = %s AND (internal_notes NOT LIKE '%[MERGED_ARCHIVE]%' OR internal_notes IS NULL) LIMIT 1",
            $external_id
        ), ARRAY_A );
        if ( ! $existing_booking ) {
            $retry_errors[] = sprintf( 'Orphan retry %s: booking no longer exists', $external_id );
            continue;
        }
        // Double-check it still has no rooms
        ...
        $raw_booking = $all_raw_by_id[ $external_id ] ?? [];
        if ( empty( $raw_booking ) ) {
            $retry_errors[] = sprintf( 'Orphan retry %s: raw API data not available', $external_id );
            continue;
        }
        ...
        $mapped_rooms = simple_hotel_crm_map_motopress_room_candidates( $raw_booking );
        if ( empty( $mapped_rooms ) ) {
            $retry_errors[] = sprintf( 'Orphan retry %s: MotoPress rooms could not be matched to any CRM room', $external_id );
            continue;
        }

        $room_rows = simple_hotel_crm_build_motopress_room_import_rows( $mapped_row, $mapped_rooms, $raw_booking );
        $room_result = simple_hotel_crm_import_booking_rooms_csv( $room_rows, false );
        if ( ! empty( $room_result['errors'] ) ) {
            $retry_errors = array_merge( $retry_errors, $room_result['errors'] );
        } else {
            $retry_imported += $room_result['created'];
        }
    }
    if ( ! empty( $retry_errors ) ) {
        $stats['errors'] = array_merge( $stats['errors'], $retry_errors );
    }
    $stats['retry_rooms'] = $retry_imported;
    ...
    $rows = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT b.id, b.guest_id, b.status_code, b.check_in_date, b.check_out_date, b.source_channel, b.total_amount, b.created_at" . ( $has_invoice_ninja_id ? ', b.invoice_ninja_invoice_id' : '' ) . ( $has_invoice_status ? ', b.invoice_status' : '' ) . ",
                    ...
                    COALESCE((SELECT SUM(brn.extras_amount) FROM {$booking_nights_table} brn JOIN {$booking_rooms_table} br2 ON br2.id = brn.booking_room_id WHERE br2.booking_id = b.id), 0) AS room_day_extras
...
function simple_hotel_crm_render_room_closures_page() {
            ...
            if ( $update_id > 0 ) {
                ...
                if ( $existing ) {
                    ...
                    if ( $wpdb->last_error ) {
                        ...
                        echo '<div class="notice notice-success"><p>' . esc_html__( 'Room closed.', 'simple-hotel-crm' ) . '</p></div>';
                    }
    ...
    if ( isset( $_POST['simple_hotel_crm_bulk_delete_closures'] ) ) {
        ...
        if ( ! empty( $ids ) ) {
            ...
            $wpdb->query( $wpdb->prepare( "DELETE FROM {$room_closures_table} WHERE id IN ({$placeholders})", $ids ) );
            if ( $wpdb->last_error ) {
                echo '<div class="notice notice-error"><p>' . esc_html( sprintf( __( 'Database error: %s', 'simple-hotel-crm' ), $wpdb->last_error ) ) . '</p></div>';
            } else {
                echo '<div class="notice notice-success"><p>' . esc_html( sprintf( __( 'Removed %d closure(s).', 'simple-hotel-crm' ), count( $ids ) ) ) . '</p></div>';
            }
            simple_hotel_crm_ics_export_refresh_all_files();
        }
        }
...
to.focus();if(to.showPicker)to.showPicker();
    ...
    if ( ! empty( $closures ) ) {
        ...
        echo '<p><label><input type="checkbox" id="select-all-closures" /> ' . esc_html__( 'Select all', 'simple-hotel-crm' ) . '</label>';
        echo ' <button type="submit" name="simple_hotel_crm_bulk_delete_closures" value="1" class="button button-small button-link-delete" onclick="return confirm(' . wp_json_encode( __( 'Delete selected closures?', 'simple-hotel-crm' ) ) . ');">' . esc_html__( 'Delete Selected', 'simple-hotel-crm' ) . '</button></p>';
        echo '<table class="widefat striped"><thead><tr>';
        echo '<th style="width:30px;"><input type="checkbox" id="select-all-closures-hdr" /></th>';
        ...
        foreach ( $closures as $closure ) {
            ...
            echo '<tr>';
            echo '<td><input type="checkbox" name="closure_ids[]" value="' . esc_attr( $closure['id'] ) . '" class="closure-cb" /></td>';
        ...
        echo '<script>
document.addEventListener("DOMContentLoaded",function(){
var sel=document.getElementById("select-all-closures");
var hdr=document.getElementById("select-all-closures-hdr");
var cbs=function(){return document.querySelectorAll(".closure-cb");};
function syncAll(){
cbs().forEach(function(cb){cb.checked=sel.checked;});
if(hdr) hdr.checked=sel.checked;
}
if(sel){sel.addEventListener("change",syncAll);}
if(hdr){hdr.addEventListener("change",function(){
cbs().forEach(function(cb){cb.checked=hdr.checked;});
if(sel) sel.checked=hdr.checked;
});}
});
</script>';
    } else {
